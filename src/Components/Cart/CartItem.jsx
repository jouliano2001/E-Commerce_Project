...
function CartItem({item}) {
  const { cart, setCart } = useContext(prodContext);
  const removeFromCart = (id) => {
    setCart(cart?.filter((item) => item?.id !== id) || []);
  };
  return (
    <div className='bg-gray-800 rounded-4xl p-4 flex justify-center align-middle items-center'>
        ...
        <img
            ...
            <div>
              <h3 className="text-2xl text-[#ffffff]">{item.title}</h3>
            ...
            <div className="flex flex-1 items-center justify-end gap-2">
              ...
              <button onClick={() => removeFromCart(item.id)} className="text-gray-600 transition hover:text-red-600">
